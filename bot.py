...
async def game_process(ctx):
    ...
    await bot.HISTORY_TEXT_CHANNEL.send(f'la nuit {bot.NB_NIGHT} tombe')
    ...
    for player in bot.DEADS:
        message += f"{player.discordMember.display_name} est mort ce soir\n"
    ...
    for player in bot.DEADS:
        if(player in bot.ALIVE_PLAYERS):
            # TODO: remove from ALIVE_PLAYERS table
            pass
        index += 1
...
async def vote(ctx, choice: int):
    ...
    if(bot.TURN == "LOUPS"):
        ...
        if(ctx.message.channel == bot.LOUPS_TEXT_CHANNEL):
            # if choice is valid
            if(choice >= 1 and choice <= len(bot.ALIVE_PLAYERS)):
                targets = [num for target.actual_num in bot.LOUP_TARGETS]
                # if the choice is not already a target
                if(choice not in targets):
                    target = Target(
                        choice, bot.ALIVE_PLAYERS[choice], ctx.message.author)
                    bot.LOUP_TARGETS.append(target)
                else:
                    ...
                    if(current_target == None):
                        raise IndexError
                    current_target.add_accusator(ctx.message.author)

                await bot.LOUPS_TEXT_CHANNEL.send(f"{bot.LOUP_TARGETS}")


async def loups_turn():

    # warn village
    await bot.HISTORY_TEXT_CHANNEL.send(f'Les loups garous ont 20 secondes pour choisir leur victime de la nuit:\n\n')
    bot.TURN = "LOUPS"
    ...
    for player in bot.ALIVE_PLAYERS:
        ...
        num += 1
    message += 'commande: !vote <int>\n'
    message += 'exemple: !vote 5\n'
    await bot.LOUPS_TEXT_CHANNEL.send(message)

    await asyncio.sleep(time_left - 10)
    time_left = 10
    await bot.LOUPS_TEXT_CHANNEL.send('10 secondes restantes')
    await asyncio.sleep(time_left - 5)
    time_left = 5
    await bot.LOUPS_TEXT_CHANNEL.send('5 secondes restantes')
    ...
    if(len(bot.LOUP_TARGETS) == 0):
        ...
        targets_choice = None
        max_accusator = max([len(target.accusator)
                             for target in bot.LOUP_TARGETS])
        targets_choice = [target for target in bot.LOUP_TARGETS if len(
            target.accusator) == max_accusator]
...
async def create_game_category(ctx):
    ...
    for player in bot.PLAYERS:
        ...
        await player.private_channel.send(player.role)
